...
data OutputFormat
  = -- | Plain text
    Plain
  | -- | Text with ANSI-terminal special characters
    Ansi
  | -- | HTML
    Html
  ...
  where
    ...
    parseFromFiles (file : rest) = do
      ...
      case parseResult of
        ...
        Right decls ->
          let decls' = if null rest then decls else filter (not . isSynthesisGoal) decls -- Remove implementations from libraries
           in ((file, decls') :) <$> parseFromFiles rest
    ...
    synthesizeGoal cquals tquals goal = do
      ...
      -- print empty
      -- print $ text "environment:"
      -- print $ vMapDoc pretty pretty (allSymbols $ gEnvironment goal)
      -- print empty
      -- print $ (text "goal:") <+> pretty (gSpec goal)
      -- print empty
      -- print $ (text "measures:") <+> vMapDoc pretty pretty (_measures $ gEnvironment goal)
      -- print empty
      -- print $ text "solution:"
