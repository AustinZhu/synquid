...
prettySType (IntersectT t1 t2) = hlParens (prettySType t1 <+> operator "&" <+> prettySType t2)
...
prettyTypeAt n t = condHlParens (n' <= n) (
  case t of
    ...
    IntersectT t1 t2 -> prettyTypeAt 0 t1 <+> operator "&" <+> prettyTypeAt 0 t2
